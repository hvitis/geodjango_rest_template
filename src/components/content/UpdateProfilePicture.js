...
class UpdateProfilePicture extends Component {
  ...
  updateProfilePicture(file) {
    ...
    axios
      .put(`${config.API_URL}/accounts/${user_id}/profile-image`, data, {
        headers: {
          accept: '*/*',
      ...
      .then(response => {
        ...
        // If upload was successful then falling fetch for getting the new IMG, cause Django adds hash on the picture.
        fetch(`${config.API_URL}/accounts/${user_id}`)
          .then(response => response.json())
          .then(
            result => {
              this.setState({
                profilePicture: result[0].profileImage.file
              });
            },
            error => {
              this.setState({
